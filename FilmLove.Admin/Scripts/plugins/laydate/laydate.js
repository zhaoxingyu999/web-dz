...
function (a) {
    ...
    c.events = function () {
        ...
        function () {
            ...
            if ("function" == typeof c.options.clear) {
                c.options.clear();
            }
        }),
        ...
        c.on(h.otoday, "click",
        function () {
            var itoday = laydate.now(0, c.options.format);
            c.elem[h.elemv] = itoday;
            c.close();
            if (!c.options.choose) {
                return;
            }
            if ("function" == typeof c.options.choose) {
                c.options.choose(itoday);
            }
        }),
        h.ok = b("#laydate_ok"),
        c.on(h.ok, "click",
